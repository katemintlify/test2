...
Command Line Interface. See [Snyk CLI](/getting-started/glossary#snyk-cli).

### Cloud Native Application Security

Implementing security throughout the CI/CD pipeline, automating security embedding in microservices and maximizing repetition to reduce the introduction of vulnerabilities. Snyk provides a comprehensive [CNAS platform](https://snyk.io/product/cloud-native-application-security/). See our article about [Cloud native security guide for building secure applications](https://snyk.io/learn/cloud-native-security-for-cloud-native-applications/).

### Container

Containers allow you to package applications and their dependencies together, to be deployed as a single runnable unit. A container is an abstraction provided by the operating system kernel, that allows a process to be isolated from other processes running on the system. Also see [Snyk Container.](/getting-started/glossary#snyk-container)
...
Dynamic Application Security Testing. An application that you can point at a site or service; it then typically profiles the site or service, then examines the output and behaviour to uncover security vulnerabilities. Also see [SAST](/getting-started/glossary#sast).
...
Can refer to a [Snyk Cloud Environment](/getting-started/glossary#snyk-cloud-environment), a [project attribute](/manage-issues/introduction-to-snyk-projects/project-attributes), or an interface for working with Snyk, such as the Snyk [CLI](/getting-started/glossary#cli), [Web UI](/getting-started/glossary#snyk-web-ui), or an [IDE](/getting-started/glossary#ide).
...
Infrastructure as Code. See [Snyk Infrastructure as Code.](/getting-started/glossary#snyk-infrastructure-as-code)

### IAST

Interactive Application Security Testing. This approach tests for vulnerabilities while running the application. See [DAST](/getting-started/glossary#dast) and [SAST](/getting-started/glossary#sast).
...
See [license policy](/getting-started/glossary#license-policy), [security policy](/getting-started/glossary#security-policy), and [.snyk policy](/getting-started/glossary#.snyk-policy).
...
Static Application Security Testing. A method to secure software by reviewing the source code of your proprietary software, and identifying sources of vulnerabilities. Also see [DAST](/getting-started/glossary#dast).
...
An organizing concept that equates to an Amazon Web Services (AWS) account, Azure subscription, or Google Cloud project. See [Snyk Cloud concepts](/scan-cloud-deployment/snyk-cloud/snyk-cloud-concepts).
...
A component powering Snyk’s cloud native application security platform. Incorporates **Snyk Intel Vulnerability DB**: Snyk’s database of vulnerabilities, providing detailed information and fix advice for known vulnerabilities. See [Vulnerability DB](https://snyk.io/vuln).

### Snyk Web UI

The environment allowing users to access Snyk functions on the web. See [Snyk Web UI](/getting-started/quickstart/create-a-snyk-account/logging-in-to-an-existing-account).
...
Representation of an external resource Snyk has scanned through an integration, the CLI, UI or API. Targets may represent a SCM repository, a Kubernetes workload, or other scannable resources external to Snyk. All [projects](/getting-started/glossary#project) that Snyk create are associated to a parent target. One target may relate to many projects. The structure of the target depends on the [origin](/getting-started/glossary#origin). See [Introduction to projects](https://docs.snyk.io/getting-started/introduction-to-snyk-projects).
